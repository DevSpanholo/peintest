...
const MIN_VIEW_TIME = 20000; // 20 segundos mínimos por página
const MAX_VIEW_TIME = 90000; // 90 segundos máximo por página
...
const SITE_URLS = [
  ...
  'https://brasilquiz.com/sorteio/participe-e-conquiste-um-playstation-5-ou-um-pc-gamer-com-o-cifra-do-bem/?utm_term=216&cf_ads=216'
];
...
async function simulateHumanBehavior(page) {
  ...
  try {
    ...
    const participateButton = await page.$('text="QUERO PARTICIPAR!"');
    
    if (participateButton) {
      console.log(chalk.yellow('🖱️ Clicando em QUERO PARTICIPAR!'));
    ...
    for (const [platform, selector] of Object.entries(AD_SELECTORS)) {
      ...
      if (ads.length > 0) {
        ...
        for (const ad of ads) {
          ...
          if (isVisible) {
            // Rola até o anúncio e pausa brevemente
